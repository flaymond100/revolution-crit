...
function navLinkClass(isActive: boolean) {
  return [
    ...
    isActive
      ? 'bg-[color:var(--surface-light)] text-[color:var(--bg-primary)]'
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[color:var(--border-dark)] bg-[color:rgba(11,15,20,0.84)] backdrop-blur-xl">
      <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
        <Link className="flex min-w-0 items-center gap-3" to="/" aria-label="Revolution Crit home">
          <img alt="Revolution Crit" className="h-10 w-10 rounded-2xl object-cover ring-1 ring-white/10" src="/assets/logo.png" />
          <div className="min-w-0">
            <p className="font-heading text-base font-semibold uppercase tracking-[0.24em] text-[color:var(--text-primary-dark)] sm:text-lg">
              Revolution Crit
            </p>
            <p className="text-xs uppercase tracking-[0.3em] text-[color:var(--accent-secondary)]">
              Race Series
            </p>
          </div>
        </Link>
        ...
        <nav aria-label="Primary" className="hidden items-center gap-1 lg:flex">
          {primaryLinks.map((link) => (
            <NavLink key={link.to} className={({ isActive }) => navLinkClass(isActive)} to={link.to}>
              {link.label}
            </NavLink>
          ))}
        </nav>
      ...
      {isMenuOpen ? (
        <div id="mobile-navigation" className="border-t border-[color:var(--border-dark)] bg-[color:var(--bg-secondary)] lg:hidden">
          <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6 sm:px-6">
            ...
            <nav aria-label="Mobile secondary" className="grid gap-2 border-t border-[color:var(--border-dark)] pt-5">
              {utilityLinks.map((link) => (
                <NavLink
                  key={link.to}
                  className={({ isActive }) =>
